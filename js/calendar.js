function initializeCalendar(buttonId, popoverId, reservationLinkId) {
        ...
        for (let i = 1; i <= lastDayOfMonth.getDate(); i++) {
            ...
            if (disabledDates.includes(i)) {
                ...
                dayElement.addEventListener('click', () => {
                    ...
                    document.querySelectorAll(`#${popoverId} .day`).forEach(d => d.classList.remove('selected'));
    ...
    document.addEventListener('click', (event) => {
        if (!calendarPopover.contains(event.target) && !openButton.contains(event.target)) {
...
document.addEventListener('DOMContentLoaded', () => {
    initializeCalendar('open-calendar-btn', 'calendar-popover', 'reservation-link');
    initializeCalendar('open-calendar-btn-clinic', 'calendar-popover-clinic', 'reservation-link-clinic');

    for (let i = 1; i <= 6; i++) {
        initializeCalendar(`open-calendar-btn-clinic-${i}`, `calendar-popover-clinic-${i}`, `reservation-link-clinic-${i}`);
    }

    for (let i = 1; i <= 6; i++) {
        initializeCalendar(`open-calendar-btn-doctor-${i}`, `calendar-popover-doctor-${i}`, `reservation-link-doctor-${i}`);
    }
});
